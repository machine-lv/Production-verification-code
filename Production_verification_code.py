...
class Creatimage(object):
    def __init__(self, font=ImageFont.truetype('./chinese_fonts/fangzheng_shusong.ttf', 30),
                 img_r=Image.new("RGB", (50, 50), (255, 255, 255)), size=(50, 50),
                 chars="我", image_arr=np.zeros((256, 256, 3), np.uint8),value=0,
                 count=0,filename="filename",image_list = [[255,240,245],[185,211,238],[187,255,255]]):
        self.font = font
        self.img_r = img_r
    ...
    def randRGB(self):
        randomcolor=[]
        randomcolor.append((255, 0,  0))  # 红
        randomcolor.append((255, 125, 0))  # 橙色
        randomcolor.append((255,255,0 )) # 黄
        randomcolor.append((0,255,0)) # 绿
        randomcolor.append((0,0,255 )) # 蓝
        randomcolor.append((0,255,255)) # 靛色
        randomcolor.append((255,0,255)) # 紫
        randomcolor.append((160,82,45))
        randomcolor.append((225, 128, 0))
        randomcolor.append((0, 0, 225))
        randomcolor.append((1,1,1)) # 黑
        #length = len(randomcolor)
        #return random.choice(randomcolor)
    ...
    def saveImage(self):
        ...
        train_images_dir = os.path.join(out_dir, self.filename)
        if not os.path.isdir(train_images_dir):
            os.makedirs(train_images_dir)
        char_dir = os.path.join(train_images_dir, "%0.5d" % self.value)
        path_image = os.path.join(char_dir, "%d.png" % self.count)
...
class FontImage(object):
    ...
    def generate_chinese_img(self, word, sample_num=20, complex_flag=True):

        def add_point_noise(thres):
            for w in range(width):
                for h in range(height):
                    tmp = random.random()
                    if tmp < thres:
                        noise_color = tuple([random.sample(range(1, 254), 1)[0] for _ in range(3)])
                        draw.point((w, h), fill=noise_color)

        def add_line_noise(num):
            for i in range(num):
                begin = (random.randint(0, width), random.randint(0, height))
                end = (random.randint(0, width), random.randint(0, height))
                noise_color = tuple([random.sample(range(1, 254), 1)[0] for _ in range(3)])
                draw.line([begin, end], fill=noise_color)

        font_list = [font_fn for font_fn in os.listdir(self.Fontdir)]
        img_list = []
        for idx in range(sample_num):
            font_fn = random.sample(font_list, 1)[0]
            size = random.randint(42, 48)
            font = ImageFont.truetype(os.path.join(self.Fontdir, font_fn), size=size)
            bg_color = [random.sample(range(1, 254), 1)[0] for _ in range(2)]
            bg_color.append(random.sample(range(128, 254), 1)[0])
            bg_color = tuple(bg_color) if complex_flag else (255, 255, 255)
            width, height = (54, 54)
            if idx==0 and complex_flag:
                img = Image.new(mode='RGB', size=(width, height), color=(255, 255, 255))
                draw = ImageDraw.Draw(img)
                draw.text(xy=(4, 4), text=word, font=font, fill=(0, 0, 0))
                img_list.append(img)
            img = Image.new(mode='RGB', size=(width, height), color=bg_color)
            img.paste()
            draw = ImageDraw.Draw(img)
            fg_color = [random.sample(range(1, 254), 1)[0] for _ in range(2)]
            fg_color.append(random.sample(range(1, 128), 1)[0])
            fg_color = tuple(fg_color) if complex_flag else (0, 0, 0)
            pos_x = random.randint(2, 8)
            pos_y = random.randint(2, 8)
            draw.text(xy=(pos_x, pos_y), text=word, font=font, fill=fg_color)
            if complex_flag:
                add_point_noise(0.1)
                add_line_noise(random.randint(0, 10))
            angle = random.randint(0, 360)
            img = img.rotate(angle)
            blur_flag = random.randint(0, 1)
            if blur_flag and complex_flag:
                img = img.filter(ImageFilter.GaussianBlur(random.random()))
                img_list.append(img)
        return img_list
...
python gen_printed_char.py --out_dir ./dataset  --font_dir ./chinese_fonts --width 30 --height 30 --margin 4 --rotate 30 --rotate_step 1
    '''
    options = args_parse()
    out_dir = os.path.expanduser(options['out_dir'])   # 输出路径
    font_dir = os.path.expanduser(options['font_dir'])  # 字体类型
    test_ratio = float(options['test_ratio'])  # 测试集的比例
    width = int(options['width'])
    height = int(options['height'])
    need_crop = not options['no_crop']
    margin = int(options['margin'])
    rotate = int(options['rotate'])  # 旋转角度；
    need_aug = options['need_aug']
    rotate_step = int(options['rot'
                              'ate_step'])
    ...
    #fontimage = FontImage(width, height, font_dir)
    number = 0
    for (char, value) in lang_chars.items():  #
        number += 1
        print("正在生产第 %d 个汉字:"%(value))
        print(char)
        allImage = 300
        for i in range(300):
            ic = Creatimage(value=value, count=i, chars=char,filename="train",image_arr=np.zeros((256, 256, 3))
                            , img_r=Image.new("RGB", (50, 50), (255, 255, 255)))
            ic.createImage(4)
            ic.saveImage()
        for i in range(30):
            ic = Creatimage(value=value, count=i, chars=char, filename="test",image_arr=np.zeros((256, 256, 3))
                            , img_r=Image.new("RGB", (50, 50), (255, 255, 255)))
            ic.createImage(4)
            ic.saveImage()
        # image_list = []
        # print (char, value)
        # image_list = fontimage.generate_chinese_img(char)
        # #char_dir = os.path.join(images_dir, "%0.5d" % value)
        # #for j, verified_font_path in enumerate(verified_font_paths):    # 内层循环是字体
        #     # !!!!! 此处到下一个感叹号是一个字符一个字体产生一个系列的数据。
        #     # if rotate == 0:
